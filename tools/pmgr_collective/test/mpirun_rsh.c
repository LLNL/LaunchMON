...
ACCEPT_HID:
	...
	alarm_msg = "Timeout during hostid exchange.\n";
        ...
        int nwrite;
        if (rank == -1) {
          /* we want to assign a rank in this case */
          nwrite = write (s1, &i, sizeof(i));  
        }
        if (nwrite == sizeof(i)) {
           rank = i;
        }
...
#if NONBLOCKING_MPIRUN
    pmgr_process_singleop (fds, nprocs, PMGR_GATHER);
#else
...
#if COMMLINE_SUPPORT
    len = sprintf(remote_command, 
    "%s --pmgrsize=%d --pmgrip=%s --pmgrport=%d --pmgrjobid=%d --pmgrlazyrank=%d", 
    remote_command, nprocs, mpirun_host, port, id, 1);
