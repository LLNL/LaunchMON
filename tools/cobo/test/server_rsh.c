 ...
 * at run-time.  These parameters are read by server_rsh from a
...
char server_host[MAX_HOST_LEN]; /* hostname of current process */
...
static char *safe_strdup(char *str)
{
    ...
    if (result == NULL) {
        fprintf(stderr, "server: strdup (%s): Out of memory\n", str);
        exit(1);
    }
    return result;
}
...
#define PARAMFILE_LEN 256
    ...
    gethostname(server_host, MAX_HOST_LEN);
...
#define DEBUGGER "gdb"
    ...
    /* build our hostlist */
    int num_hosts = nprocs;
    char** hostlist = (char**) malloc(nprocs * sizeof(char*));
    for (i = 0; i < nprocs; i++) {
        hostlist[i] = plist[i].hostname;
    }

    /* open the server */
    cobo_server_open(2384932, hostlist, num_hosts, portlist, num_ports);
...
int start_process(int i, char *command_name, char *env)
{
    ...
    if (ld_library_path != NULL ) {
        sprintf(remote_command, "cd %s; %s LD_LIBRARY_PATH=%s:%s "
                ...
                display,env,device_port_env);
    } else {
        sprintf(remote_command, "cd %s; %s LD_LIBRARY_PATH=%s "
                "%s %s %s",
                wd, ENV_CMD,LD_LIBRARY_PATH_MPI,
                display,env,device_port_env);
    }
    ...
    if (xterm_on) {
        ...
        sprintf(xterm_title, "\"server process %d of %d\"", i, nprocs);
    }
...
ACCEPT_WFE:
	...
	{
		printf("server_rsh: Abort signaled from [%d]\n",local_id);
                        close(s);
                        close(s1);
                        cleanup();
...
void process_termination()
{
    ...
    if (mcs_read != GRP_INFO_LEN) {
        fprintf(stderr, "server_rsh: read grp info failed "
                "(len %d %s)\n", mcs_read, grp_info);
        cleanup();
    }
    ...
    for (i = 1; i < nprocs; i++) {
        ...
        if (mcs_written != GRP_INFO_LEN) {
            fprintf(stderr, "server_rsh: write grp info failed %d\n",
                    mcs_written);
            cleanup();
        }
    }
    ...
    for (i = 1; i < nprocs; i++) {
        ...
        if (mcs_read != sizeof(my_rank)) {
            fprintf(stderr, "server_rsh: write grp info failed %d\n",
                    mcs_written);
            perror("server_rsh: read  grp ack");
            cleanup();
        }
    }
    ...
    if (mcs_written != sizeof(my_rank)) {
        perror("cobo_exchange_mcs_group_sync: server_rsh "
               "write ack to root");
        cleanup();
    }
#endif
...
void usage(void)
{
    fprintf(stderr, "usage: server_rsh [-v] [-rsh|-ssh] "
            "[-paramfile=pfile] "
...
void child_handler(int signal)
{
    ...
    if (use_totalview) {
	fprintf(stderr, "server: child died. Waiting for others.\n"); 
    }
    alarm(10);
